...
usage() {
  cat <<'EOF' >&2
Usage: farcaster-thread.sh --fid <fid> --hash <hash> [--limit <0-200>] [--bot <name>]

Returns the thread rooted at the supplied cast via /api/vibeshift/thread.
EOF
  exit 1
}
...
if [[ -z "$fid" || -z "$hash" ]]; then
  echo "fid and hash are required" >&2
  usage
fi
...
params=(--data-urlencode "fid=$fid" --data-urlencode "hash=$hash")
...
http_status="$(curl --fail --show-error -sS \
  -G "${base_url}/api/vibeshift/thread" \
  "${params[@]}" \
  -H "x-api-key: ${api_key}" \
