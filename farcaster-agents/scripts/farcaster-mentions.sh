...
usage() {
  cat <<'EOF' >&2
Usage: farcaster-mentions.sh --target-fid <fid> [--limit <1-200>] [--cursor <cursor>] [--bot <name>]

Returns mentions whose target is the supplied fid via /api/vibeshift/mentions-to-target.
EOF
  exit 1
}
...
curl --fail --show-error -sS \
  -G "${base_url}/api/vibeshift/mentions-to-target" \
  "${params[@]}" \
  -H "x-api-key: ${api_key}"
