...
usage() {
  cat <<'USAGE' >&2
Usage: vibeshift-feed-affinity.sh --fid <fid> [--limit <1-200>] [--cursor <cursor>] [--deleted <true|false>] [--only-deleted <true|false>] [--affinity-limit <n>] [--bot <name>]

Fetches /api/vibeshift/feedAffinity with optional pagination and affinity limit.
USAGE
  exit 1
}
...
curl --fail --show-error -sS \
  -G "${base_url}/api/vibeshift/feedAffinity" \
  "${params[@]}" \
  -H "x-api-key: ${api_key}"
